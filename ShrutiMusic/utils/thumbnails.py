...
async def gen_thumb(videoid: str):
    ...
    try:
        ...
        # HD thumbnail (maxres → hq fallback)
        thumb_urls = [
        ...
        try:
            async with aiohttp.ClientSession() as session:
                for thumburl in thumb_urls:
                    try:
                        async with session.get(thumburl, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                            if resp.status == 200:
                                data = await resp.read()
                                # maxres kabhi 120x90 placeholder bhejta hai
                                if len(data) > 5000:
    ...
    try:
        ...
        if layout["show_particles"]:
            ...
            # blur hata diya taaki image soft na lage

        art_size = layout["art_size"]
        art_x = layout["art_x"]
        art_y = (CANVAS_H - art_size) // 2

        # cover fit (crop center) — stretch nahi
        bw, bh = base_img.size
        side = min(bw, bh)
        left = (bw - side) // 2
        top = (bh - side) // 2
        base_img = base_img.crop((left, top, left + side, top + side))
        ...
        if random.choice([True, False]):
            add_glow_ring(canvas, art_x, art_y, art_size, accent_color, random.randint(6, 12))
        ...
        draw.line(
            [(brand_x, underline_y), (brand_x + brand_w, underline_y)],
        ...
        np_shadow = 3
        draw.text((info_x + np_shadow, np_y + np_shadow), np_text, fill=(0, 0, 0, 180), font=np_font)
        ...
        title_shadow = 2
        draw.multiline_text(
            (info_x + title_shadow, title_y + title_shadow),
            title_text,
            fill=(0, 0, 0, 160),
            font=title_font,
        ...
        if random.choice([True, False]):
            ...
            draw.line(
                [(CANVAS_W - 25, 25), (CANVAS_W - 25 - corner_size, 25)],
                fill=corner_color,
                width=corner_width,
            )
            draw.line(
                [(CANVAS_W - 25, 25), (CANVAS_W - 25, 25 + corner_size)],
                fill=corner_color,
                width=corner_width,
            )
